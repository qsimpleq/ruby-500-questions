...
### Тестирование
  1.  Знаете ли вы, что такое Load Testing?\
      Чем может помочь разработчик при Load Testing?
  2.  Какие инструменты используют для того, чтобы собрать как можно больше метрик при Load Testing?
  3.  Какие подходы вы бы порекомендовали использовать, чтобы ускорить тесты на CI?
  4.  Как внедрить процесс написания тестов, если проект (Rails API) их совсем не имеет?

### Практические задания
  1. Написать реализацию структуры данных Set.\
     Объяснить алгоритмическую сложность добавления элементов к сету.
  2. Спроектировать реферальную систему (то есть, пользователь может пригласить других пользователей,\
